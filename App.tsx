...
import { getChatResponseStream, findProjects } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (prompt: string, image?: { mimeType: string; data: string }) => {
    ...
    try {
      ...
      const stream = getChatResponseStream(prompt, history, systemInstruction, image, isThinkingMode);
      ...
      for await (const chunk of stream) {
        ...
        setMessages(prev => {
            ...
            // Update the last message if it is the model message
      ...
      setMessages(prev => {
          const newMessages = [...prev];
          // If the last message was the one we were streaming into (and it's empty), replace it.
          // Otherwise append the error.
          const lastIndex = newMessages.length - 1;
          if(newMessages[lastIndex].role === 'model' && !newMessages[lastIndex].parts[0].text) {
